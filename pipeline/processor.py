...
def run_pipeline(
    ...
    for sub in data.get("subsidiaries", []):
        try:
            ...
            logger.debug("Could not save subsidiary %r: %s", sub.get("name"), exc)
    ...
    for plant in data.get("plants", []):
        try:
            ...
            logger.debug("Could not save plant %r: %s", plant.get("name"), exc)
    ...
    for contact in data.get("contacts", []):
        ...
        try:
            ...
            logger.debug("Could not save contact %r: %s", contact.get("full_name"), exc)
